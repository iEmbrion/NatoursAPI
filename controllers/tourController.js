...
//If only have 1 field use .array()
...
// /tours-within/233/center/34.111734, -118.113491/unit/mi
exports.getToursWithin = catchAsync(async (req, res, next) => {
  const { distance, latlng, unit } = req.params;
  const [lat, lng] = latlng.split(',');

  //Get Radiant based on unit (miles / km)
  const radiant = unit === 'mi' ? distance / 3963.2 : distance / 6378.1;

  if (!lat || !lng) {
    next(new AppError('Please provide in format of lat,lng.', 400));
  }
